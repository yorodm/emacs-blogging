...
#+date: <2015-03-08>
#+keywords: kernel, 5129, card reader, linux
#+setupfile: ../org-templates/post.org

/Updated on March 24, 2020/

The post explains how to enable *Realtek RTS5129 Card Reader* controller in the
Linux kernel.
...
log but no luck. OK! that's fine, I looked into =lspci= output but still not
trace of Card Reader Controller. May be the controller is not attached to PCI
bus. I fired =lsusb= and voila! I got an entry. The output had a reference of
...
#+CAPTION: Linux kernel 3.19.0. Result of 'RTS'
#+ATTR_HTML: :alt Linux kernel 3.19.0. Result of 'RTS' :title Linux kernel 3.19.0. Result of 'RTS'
#+ATTR_HTML: :width 70% :height
[[file:images/posts/rtsx/search_results_of_rts.png]]
...
#+CAPTION: Enable MFD RTSX USB
#+ATTR_HTML: :alt Enable MFD_RTSX_USB :title Enable MFD_RTSX_USB
#+ATTR_HTML: :width 100% :height
[[file:images/posts/rtsx/realtek_usb_card_reader.png]]
...
Still the SD-card was not detected. Something was still missing. I went ahead
and searched the Linux kernel config for 'MMC'(MultiMediaCard) and two familiar
modules showed up.

#+CAPTION: Linux kernel 3.19.0. Result of 'MMC'
#+ATTR_HTML: :alt Linux kernel 3.19.0. Result of 'MMC' :title Linux kernel 3.19.0. Result of 'MMC'
#+ATTR_HTML: :width 80% :height
[[file:images/posts/rtsx/search_results_of_mmc.png]]

Again I safely ignored =MMC_REALTEK_PCI= and enabled =MMC_REALTEK_USB= from
Device Drivers -> MMC/SD/SDIO card support -> Realtek USB SD/MMC Card interface
Driver as module.

#+CAPTION: Enable MMC REALTEK USB
#+ATTR_HTML: :alt Enable MMC_REALTEK_USB :title Enable MMC_REALTEK_USB
#+ATTR_HTML: :width 100% :height
[[file:images/posts/rtsx/realtek_usb_sd-mmc_card_interface_driver.png]]

I compiled the kernel again and re-inserted the SD-card. This time the
...
/Note/: The module =rtsx_usb= should load automatically. If the module is not
able to load, create a file =rtsx_usb.conf= inside =/etc/modules-load.d/= with
following content:
#+BEGIN_SRC shell -n
