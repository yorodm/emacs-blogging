...
Mostly I use Emacs available from the master branch and install the binary in
=~/bin/=. Below are the steps:
#+BEGIN_SRC shell -n
  ./configure --with-mailutils --prefix=$HOME/source/emacs/build --bindir=$HOME/bin
  make -j 4			# Where 4 is the job count that can run simultaneously.
  make install
#+END_SRC
