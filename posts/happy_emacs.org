...
#+date: <2016-04-17>
#+filetags: emacs pretest
#+setupfile: ../org-templates/post.org
#+options: toc:t

** Testing the pretest version

I decided to use the pretest version of Emacs instead of a stable release and so
far it turned out to be a pleasant experience.
...
** Fedora 31 notes(Sep 07, 2020)
   - Install required packages
     #+BEGIN_SRC shell -n
       ...
       dnf install texinfo
       dnf install libXpm-devel libjpeg-turbo-devel
       dnf install libxml2-devel
       dnf install giflib-devel gnutls-devel
   ...
   - Clone repos
     #+BEGIN_SRC shell -n
       cd $HOME/source
       git clone https://github.com/emacs-mirror/emacs.git --depth 2
       cd emacs

       # To display ERC channel name in the desktop notification(Optional)
       git remote add psachin https://github.com/psachin/emacs.git
       git fetch psachin --depth 2
       git checkout -b erc-show-channel-in-notification psachin/erc-show-channel-in-notification
       git rebase master
     #+END_SRC
   ...
   - Compile
     #+BEGIN_SRC shell -n
       ...
       ./autogen.sh
       ./configure --host=x86_64-fedora31-gnu --with-mailutils --prefix=$HOME/source/emacs/build --bindir=$HOME/bin --with-x-toolkit=yes
       make -j 10
       make install
     #+END_SRC
